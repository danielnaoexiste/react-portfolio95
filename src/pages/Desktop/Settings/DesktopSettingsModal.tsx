...
export const DesktopSettingsModal = () => {
  const { setShowSettings, theme, setTheme } = useDesktopContext()
  ...
  return (
    <Modal
      ...
      defaultPosition={{ x: 870, y: 192 }}
      icon={<Settings variant="32x32_4" />}
      closeModal={() => setShowSettings(false)}
      buttons={[{ value: t("actions.ok"), onClick: () => setShowSettings(false) }]}
    >
      <Frame h="100%" boxShadow="none" padding={2}>
        ...
        >
          <div style={{ marginBottom: ".5rem" }}>{t("settingsModal.scheme")}</div>
          <Dropdown
            options={options}
